...
1. **Старт:** Автоматизация следит за энергопотреблением розетки/датчика. Если мощность выше порога (например, 10Вт) более заданного времени — цикл считается начавшимся.
2. **Процесс:** При старте опционально отправляется уведомление "Стирка началась".
3. **Финиш:** Когда мощность падает ниже порога и держится так заданное время — стирка завершена.
4. **Расчёт:** Разница показаний Utility Meter (кВт·ч) умножается на выбранный тариф (дн/ночь — выбирается в зависимости от времени окончания).
5. **Напоминания:** В Телеграм приходят кнопки "Напомнить через 30/60/120 мин". Если не нажали — автоматика сама напомнит заданное количество раз.
</details>

<details>
<summary>⚠️ Настройки (Helpers) 👈</summary>
Для полноты функций создайте (Settings → Devices → Helpers) или укажите существующие:
- `input_number.stirka_start_energy` — хранит показание счётчика на старте (input_number).
- Utility Meter / sensor в кВт·ч (`sensor.utility_meter_washing_kwh`), который считает кВт·ч (нужен для расчёта стоимости).
- `input_number.day_tariff`, `input_number.night_tariff` — тарифы в вашей валюте за 1 кВт·ч.
- Telegram notify (напр. `notify.telegram`) и чат/target для отправки сообщений.
</details>
